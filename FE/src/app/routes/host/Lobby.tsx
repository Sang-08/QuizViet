...
export default function HostLobby() {
  ...
  const [sessionId] = useState("449 4586"); // Mock session ID

  console.log("HostLobby - quizId:", quizId);

  // TODO: Fetch quiz data from API using quizId
  // const { data: quizData } = useQuery(['quiz', quizId], () => fetchQuiz(quizId));
  const [quizTitle] = useState("Phương trình bậc hai"); // Mock - sẽ lấy từ API
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-400 via-pink-400 to-purple-500 relative overflow-hidden">
      ...
      >
        <div className="text-center py-6">
          ...
          <p className="text-gray-600 mb-2">Quét mã để tham gia</p>
          <p className="text-2xl font-bold text-gray-900">{sessionId}</p>
        </div>
      </Modal>
